...
user_db = client['Users']  # Using the 'Users' database
credentials_collection = user_db['Credentials']  # Collection for storing user credentials
...
# Sign-up function
def sign_up_user(username, password, gender):
    hashed_password = hash_password(password)
...
if not st.session_state.logged_in:
    ...
    # Check if all fields are filled before enabling buttons
    if username and password and gender != "Select your gender":
        if st.button("Sign Up"):
            ...
            if credentials_collection.find_one({"username": username}):
                ...
                # Sign up the user
                sign_up_user(username, password, gender)
        ...
        st.info("Please fill out all fields to enable sign up and login.")

# Gender-Specific Prediction Page
else:
    # Load the prediction view based on gender
    styled_header(f"Welcome {st.session_state.username}, Questionaire for {st.session_state.gender} Patients")
